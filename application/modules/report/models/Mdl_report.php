...
//                 WHERE DATEDIFF(CURRENT_DATE, STR_TO_DATE(`t`.`birthdate`, '%Y-%m-%d'))/365 >= '60'
	...
	public function m_count_pwds(){
		$sql = "SELECT
						...
						COUNT(IF(`t`.`sexid` = '1',`t`.`masterid`,NULL)) AS `male`,
						COUNT(IF(`t`.`sexid` = '2',`t`.`masterid`,NULL)) AS `female`,
                                        (COUNT(IF(`t`.`sexid` = '1',`t`.`masterid`,NULL))+
						COUNT(IF(`t`.`sexid` = '2',`t`.`masterid`,NULL))) AS `total`        
	          FROM
	            `t_master` `t`
	          INNER JOIN `m_brgy` `b`
	            ON `t`.`brgyid` = `b`.`brgyid`
	...
	public function m_countSoloParent(){
		$sql = "SELECT 
                            (COUNT(IF(`t`.`sexid` = '1',`t`.`masterid`,NULL)) +
	           COUNT(IF(`t`.`sexid` = '2',`t`.`masterid`,NULL))) AS `total`
	          FROM
	...
	public function m_get_count_fernandinos(){
		$sql = "SELECT
			(COUNT(IF(`t`.`statusid` = '1',`t`.`masterid`,NULL)) +
			COUNT(IF(`t`.`statusid` = '2',`t`.`masterid`,NULL)) +
			COUNT(IF(`t`.`statusid` = '3',`t`.`masterid`,NULL)) +
			COUNT(IF(`t`.`statusid` = '4',`t`.`masterid`,NULL))) AS 'fernandinos'
			FROM `t_master` `t`";
		$query = $this->db->query($sql);
		if($query->num_rows() > 0){
			return $query->result();
	...
	public function m_del_orgmemebers($orgmemberid,$data){
		$this->db->where('`id`',$orgmemberid);
		$query = $this->db->update('tbl_orgmembers',$data);
		if($query){
			return true;
	...
	public function m_get_orgmembers($orgid){
		$sql = "SELECT
						...
						IF(`a`.`masterid` != '0',
							CONCAT(`b`.`lname`,', ',`b`.`fname`,' ',`b`.`mname`),
							`name`
						)AS `fullname`,
						`b`.`idNo`,
						`a`.`designation`,
