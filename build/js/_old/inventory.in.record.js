$(function() {
  ...
  get_json(base_url + "inventory/ajax_get_risno?str=I-", "#col0_filter option", "#col0_filter");
  get_json(base_url + "inventory/ajax_get_location?str=I-", "#col1_filter option, #col2_filter option", "#col1_filter, #col2_filter");
  get_json(base_url + "inventory/ajax_get_fund?str=I-", "#col3_filter option", "#col3_filter");
  get_json(base_url + "inventory/ajax_get_program?str=I-", "#col4_filter option", "#col4_filter");
  ...
  var otable = $('#dt_in_record').DataTable({
    "ajax": {
      "url": base_url + "inventory/ajax_record_in",
      "dataSrc": ""
    },
    "columns": [{
        "data": "tr_rs_no"
    ...
    'fnCreatedRow': function(nRow, data, iDisplayIndex) {
      ...
      if (data.tr_supplier) { $(nRow).attr('data-tr_supplier', data.tr_supplier); } else { $(nRow).attr('data-tr_supplier',''); }
      $(nRow).attr('data-addedby', data.a_name);
    },
    dom: '<"wrapper"Bfit>',
    'info': false,
    'order': [
  ...
  //PRINT PREVIEW
  $('#dt_in_record tbody').on('click', 'tr', function() {
    ...
    if ($('body .ui-pnotify > .alert-info').length == 0) {
      if (typeof attr !== typeof undefined && attr !== false) {
        ...
        $("#mdl-print-draft .modal-body p:eq(0)").text($(this).data('tr_supplier'));
        $("#mdl-print-draft .modal-body p:eq(8)").text($(this).data('addedby'));
  ...
  $("body").delegate("#btn_record_print", "click", function() {
    ...
    if ($("#mdl-print-draft .modal-body p:eq(8)").text() == "CIELO SALONGA") {
  ...
  function filterColumn(i) {
    $('#dt_in_record').DataTable().column(i).search($('#col' + i + '_filter').val()).draw();
  }
